...
class ChapterGenTest:
    ...
    def step_create_project(self):
        ...
        if opts.count() > 0:
            opts.nth(0).click()
            self.log("选择题材：第1项", "pass")
        if opts.count() > 4:
            opts.nth(4).click()
            self.log("选择基调：第5项", "pass")

        # 填作者名
        author = self.page.locator("input[placeholder*='作者']")
        if author.count() > 0:
            author.fill("测试作者")
            self.log("填写作者名", "pass")
        ...
        if cp:
            ...
            self.log("项目创建后 currentProject 为空，强制加载...", "info")
    ...
    def step_open_file_and_write(self):
        step = self.next_step()
        self.log(f"步骤{step}：打开文件并填写内容", "step")
        self.close_modal()
        self.page.wait_for_timeout(1500)
        ...
        try:
            # 展开第一个目录
            first_folder = self.page.locator(".tree-item.folder").first
            if first_folder.count() > 0:
                first_folder.click()
                self.page.wait_for_timeout(800)
                self.log("展开第一个目录", "pass")

            # 点击第一个文件
            first_file = self.page.locator(".tree-item:not(.folder)").first
            if first_file.count() > 0:
                first_file.click()
                self.page.wait_for_timeout(800)
                self.log("点击第一个文件", "pass")
            else:
                # 尝试点击文件树中的任何文件
                file_items = self.page.locator(".tree-item")
...
"""
    ...
    def step_use_ai(self):
        step = self.next_step()
        self.log(f"步骤{step}：使用 AI 助手", "step")
        self.close_modal()
        ...
        if sb.count() > 0:
            ...
            if len(msgs) >= 2:
                ...
                self.log(f"聊天消息数量：{len(msgs)}，AI 可能未响应", "info")
        else:
    ...
    def step_verify(self):
        step = self.next_step()
        self.log(f"步骤{step}：最终验证", "step")
        self.close_modal()

        cp = self.js("window.$moyun.currentProject?.value?.name || '无'")
    ...
    def run(self):
        ...
        try:
            ...
            self.log("=" * 60, "info")
            self.log(f"测试完成：通过 {passed}，失败 {failed}", "info")
            return failed == 0
