"""
墨韵 - Playwright 端到端测�?模拟人类操作测试所有功能，并用 LLM 验证生成内容
"""
import asyncio
import json
import sys
...
class MoyunE2ETest:
    ...
    async def start(self):
        ...
        self.browser = await self.playwright.chromium.launch(
            headless=False,  # 显示浏览�?            slow_mo=300      # 减慢操作以便观察
        )
        ...
        try:
            ...
            self.errors.append(f"测试主流程异�? {str(e)}")
        finally:
    ...
    async def test_all_features(self):
        ...
        # 3. 创建新项�?        await self.test_create_project()
    ...
    async def test_homepage(self):
        ...
        try:
            ...
            # 检查标�?            title = await self.page.title()
            print(f"页面标题: {title}")
            assert "墨韵" in title, "页面标题不正�?
    ...
    async def test_open_project_modal(self):
        ...
        try:
            ...
            # 检查弹�?            await expect(self.page.locator("text=打开项目").first).to_be_visible()
    ...
    async def test_create_project(self):
        ...
        try:
            ...
            # 检查创建项目弹�?            await expect(self.page.locator("text=创建项目").first).to_be_visible()
            
            # 填写表单
            await self.page.fill("[placeholder*='名称'], [placeholder*='name'], input[name='name']", TEST_PROJECT_NAME)
            await self.page.fill("[placeholder*='作�?], [placeholder*='author'], input[name='author']", TEST_AUTHOR)
            ...
            # 点击下一�?创建按钮
            next_btn = self.page.get_by_text("下一�?, exact=False).or_(self.page.get_by_text("创建", exact=False)).first
    ...
    async def test_enter_project(self):
        ...
        try:
            ...
            if await test_project.count() > 0:
                ...
                # 检查项目是否打开成功
                # 查找编辑器或文件�?                await expect(self.page.locator("text=outline.md").first).to_be_visible()
                
                print("�?成功进入项目")
                self.passed_tests.append("进入项目")
            else:
                print("⚠️  未找到测试项目，尝试进入已有项目")
                # 点击第一个项�?                first_project = self.page.locator(".project-item, li").first
    ...
    async def test_edit_outline(self):
        ...
        try:
            ...
            if await outline_file.count() > 0:
                ...
                # 查找编辑�?                editor = self.page.locator("textarea, [contenteditable='true']").first
                if await editor.count() > 0:
                    ...
                    # 触发保存（点击保存按钮或自动保存�?                    save_btn = self.page.get_by_text("保存", exact=False).first
    ...
    async def test_llm_generation(self):
        ...
        try:
            ...
            if await generate_btn.count() > 0:
                ...
                # 查找提示词输入框
                prompt_input = self.page.locator("textarea, input[type='text']").filter(has_text="请输�?).first
                if await prompt_input.count() == 0:
                    # 尝试查找其他输入�?                    prompt_input = self.page.locator("textarea, input[type='text']").first
                
                if await prompt_input.count() > 0:
                    await prompt_input.fill("写一段关于主角初入修仙界的场�?)
                    
                    # 点击生成按钮
                    submit_btn = (
                        self.page.get_by_text("生成", exact=True)
                        .or_(self.page.get_by_text("发�?, exact=False))
    ...
    async def test_quality_assurance(self):
        ...
        try:
            ...
            if found_features:
                print(f"�?质量保障功能正常 (发现 {len(found_features)} �?")
        ...
        try:
            ...
            if projects_path.exists():
                ...
                if project_dirs:
                    ...
                    print(f"检查项�? {latest_project.name}")
                    
                    # 查找生成的文�?                    files_to_check = [
                        ...
                        if file_path.exists():
                            ...
                            if len(content) > 50:
                                ...
                                if is_valid:
                                    ...
                                    print(f"⚠️  {file_path.name} 内容需要检�?)
                    else:
    ...
    async def llm_verify_content(self, content: str, filename: str) -> bool:
        ...
        try:
            ...
            verify_prompt = f"""请检查以�?{filename} 的内容是否合理：
...
请回答：
1. 内容是否完整�?2. 内容是否符合小说创作的要求？
3. 格式是否正确�?
只需回答 '通过' �?'不通过'，然后简短说明理由�?""
