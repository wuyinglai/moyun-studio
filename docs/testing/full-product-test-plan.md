...
1. 进入 `/project/:projectId`。
2. 打开文件树中的 `sec-001.md`。

期望：

- 编辑器读取正文。
- toolbar 显示“写下一场景”“润色”“重写”等适用按钮。
- 右侧面板可切换 Candidate / Workflow / Memory 等 tab。
...
- 如果 `sec-002.md` 不存在或为空，可以写入。
- 如果 `sec-002.md` 已有正文，不得静默覆盖，应生成 candidate 或提示确认。
- SSE 流式显示生成进度。
- 结束后重新从后端读取目标文件。
- 文件树刷新或目标文件可见。
