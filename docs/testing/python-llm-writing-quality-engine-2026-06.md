...
### Phase T3-D7.5.2：Plot Debt 实体提取降噪 ✅
- **优化内容**：减少错误实体，提升实体提取准确性
- **降噪策略**：
  - 优先匹配已知有效实体列表
  - 过滤包含动词的短语（如"李玄推门走进墨香阁"）
  - 过滤以"深夜"、"映着"、"这么晚"等开头的噪声短语
  - 限制实体长度（2-6字）
  - 禁止的噪声实体：深夜书、映着昏黄的灯、李玄推门走进墨香、李玄推门走进墨香阁、这么晚了还来买书
- **关键设计原则**：**实体准确性优先于填充率**，宁可 entity 为空也不填错误实体
- **降噪后实体提取结果**：
  - ✅ 玄黄秘录
  - ✅ 玄铁令牌
  - ✅ 青铜灯
