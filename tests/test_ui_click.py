...
async def test_1_page_load(page):
    ...
    try:
        ...
        # 检查核心元素
        header = await page.locator("text=墨韵").count() > 0
...
async def test_2_settings(page):
    ...
    try:
        ...
        # 检查弹窗
        modal = await page.locator('.modal-overlay, .modal, [role="dialog"]').count() > 0
        record("设置弹窗可见", modal)
        
        # 检查DeepSeek是否在Provider选项中
        ...
        # 填写API配置
        # API Key
        await fill_input(page, 'input[type="password"], input[placeholder*="Key"], input[name*="key"]', "sk-4ea45b73004f44a98c2f472d354430d1")
        await asyncio.sleep(0.5)
...
async def test_3_create_project_wizard(page):
    ...
    try:
        ...
        # 步骤3: 选择写作风格 - 快节奏
        await wait_and_click(page, 'button:has-text("快节奏")')
        await asyncio.sleep(0.5)
        record("选择写作风格-快节奏", True)
        ...
        # 点击生成书名
        clicked = await wait_and_click(page, 'button:has-text("生成"), button:has-text("开始")')
        ...
        if clicked:
            ...
            for i in range(45):
                ...
                if "《" in content and "》" in content:
                    print(f"  AI生成完成 (等待{i+1}秒)")
                    generated = True
                    break
                if i % 10 == 0 and i > 0:
                    print(f"  已等待 {i} 秒...")
            
            record("AI生成书名创意", generated, "等待最多45秒")
        ...
        # 点击下一步
        await wait_and_click(page, 'button:has-text("下一步")')
        await asyncio.sleep(1)
        record("点击下一步", True)
...
async def test_5_editor(page):
    ...
    try:
        ...
        record("工具栏按钮存在", count > 0, f"按钮数量: {count}")
...
async def test_6_chat_panel(page):
    ...
    try:
        ...
        if chat_input:
            ...
            # 点击发送
            await wait_and_click(page, 'button:has-text("发送"), .send-btn')
            await asyncio.sleep(3)
            record("发送聊天消息", True)
