...
跳过: 需�?LLM API Key 的端点（quality review, batch generate, extract�?
...
def test_list_projects():
    ...
    check("响应�?, code == 200, f"got {code}")
    check("success", data.get("success") is True)
    check("�?projects", "projects" in data.get("data", {}))
...
def test_create_file():
    ...
    check("响应�?, code in (200, 201), f"got {code}")
    check("success", data.get("success") is True)
...
def test_get_tree():
    ...
    check("有目录节�?, len(chapters) > 0)

test("GET /api/tree �?文件�?, test_get_tree)
...
def test_rename_file():
    ...
    check("响应�?, code == 200, f"got {code}")
    check("success", data.get("success") is True)
    # 改回�?    api("/file/rename", method="POST", data={
        "project_id": PROJECT_ID,
        "old_path": new_path,
        "new_path": FILE_PATH,
    })

test("POST /api/file/rename �?重命名文�?, test_rename_file)
...
test("GET /api/story-state/:id �?读取故事状�?, test_story_state)
...
test("POST /api/story-state/:id �?更新故事状�?, test_update_story_state)
...
def test_characters():
    ...
    check("响应�?, code == 200, f"got {code}")
    check("�?characters", "characters" in data.get("data", {}))
...
test("GET /api/recent-context/:id �?读取近期上下�?, test_recent_context)
...
test("POST /api/recent-context/:id/append �?追加上下�?, test_recent_context_append)


def test_tokens_count():
    data, code = api("/tokens/count", method="POST", data={
        "text": "这是一段用于测�?token 计量的文本�? * 10,
    })
    check("响应�?, code == 200, f"got {code}")
    check("�?tokens", "tokens" in data.get("data", {}))
...
def test_compare():
    ...
    check("响应�?, code == 200, f"got {code}")
    check("�?diff", "diff" in data.get("data", {}))
...
test("GET /api/prompts �?提示词列�?, test_prompts)
...
def test_batch_review_no_key():
    ...
    # 没有 API Key 时可能返�?500 �?503，不是空响应就行
    print(f"      -> 响应�? {code}", end="")
    if code in (200, 201):
