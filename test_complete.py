    ...
    async def test_project_management(self):
        ...
        try:
            ...
            if resp.status_code == 200:
                data = resp.json()
                print(f"   ✅ 成功，当前有 {data.get('data', {}).get('total', 0)} 个项目")
            ...
            if resp.status_code == 200:
                ...
                print(f"   ✅ 成功")
                print(f"      项目名称: {data.get('data', {}).get('name')}")
                print(f"      完成度: {data.get('data', {}).get('completion_rate', 0) * 100:.1f}%")
    ...
    async def test_file_operations(self):
        ...
        try:
            ...
            if len(found_files) == len(expected_files):
                self.passed_tests.append("检查初始文件")
            else:
                self.failed_tests.append("检查初始文件")
...
"""
            ...
            print(f"   ✅ 成功写入 (新长度: {len(new_content)} 字符)")
            ...
            if sg_path.exists():
                ...
                self.passed_tests.append("检查文风指南")
            else:
                print(f"   ⚠️  文风指南不存在")
    ...
    async def test_llm_generation(self):
        ...
        try:
            ...
            try:
                ...
                print(f"   ✅ LLM 调用成功 (耗时: {elapsed:.1f}秒)")
                print(f"   生成内容长度: {len(result)} 字符")
                
                # 保存生成的内容
                chapter_file = Path(f"workspace/projects/{self.project_id}/chapters/test_chapter.md")
        ...
        try:
            ...
            for filename, desc in quality_files.items():
                ...
                if filepath.exists():
                    ...
                    self.passed_tests.append(f"检查 {desc}")
                else:
                    print(f"\n⚠️  {desc} ({filename}) 不存在")
            ...
            for d in [revision_dir, feedback_dir]:
                if d.exists():
                    print(f"\n✅ {d.name} 目录存在")
                    self.passed_tests.append(f"检查 {d.name} 目录")
                else:
                    print(f"\n⚠️  {d.name} 目录不存在")
    ...
    async def verify_generated_content(self):
        ...
        try:
            ...
            for filepath in self.generated_files:
                ...
                # 构建验证提示
                verify_prompt = f"""请评估以下小说创作相关的文件内容，这是文件: {path.name}
...
请从以下几个方面评估（只需回答分数 0-10 和简短评价）：
1. 内容完整性 (是否结构完整)
2. 内容质量 (是否符合小说创作要求)
3. 格式规范性 (格式是否正确)

请以 JSON 格式回答，格式如下:
{{"overall_score": 分数, "comments": "简短评价"}}
"""
                ...
                try:
                    ...
                    try:
                        ...
                        print(f"   ✅ 内容已生成 (长度: {len(content)} 字符)")
                        
                except Exception as e:
                    print(f"   ⚠️  验证失败: {e}")
            
            print(f"\n验证完成，共验证 {verified_count} 个文件")
    ...
    async def print_report(self):
        ...
        if self.generated_files:
            print(f"\n📝 生成/修改的文件 ({len(self.generated_files)}):")
        ...
        if not self.failed_tests and not self.errors:
            ...
            print(f"⚠️  有 {len(self.failed_tests)} 个测试失败，{len(self.errors)} 个错误")
        print(f"{'=' * 80}")
