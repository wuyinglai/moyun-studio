...
"""
    ...
    try:
        candidate_resp = requests.post(
            "http://localhost:8000/candidates/demo-novel",
            json={
    ...
    if candidate_info:
        ...
        if candidate_info.get("action") == "chat_selected_text":
    ...
    try:
        ...
        if list_resp.status_code == 200:
            ...
            for idx, c in enumerate(list_data.get('candidates', [])):
                print(f"  [{idx+1}] id={c.get('id')}, source={c.get('source_path')}")
    except Exception as e:
    ...
    try:
        file_candidates = requests.get(
            f"http://localhost:8000/candidates/demo-novel/file/{TEST_FILE_PATH}",
            timeout=10
        )
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        f.write("# T4.7.2: ChatPanel Selected Text + Candidate Link - 轻量测试\n\n")
        ...
        f.write("- ✅ **内容包含选中文字** - candidate 内容里包含选中的文字\n")
