"""
墨韵AI小说创作助手 - 完整UI自动化测�?使用Playwright模拟用户操作测试所有功�?"""
...
async def test_ui_flow():
    ...
    print("=" * 80)
    print("墨韵AI小说创作助手 - 完整UI自动化测�?)
    print("=" * 80)
    
    async with async_playwright() as p:
        # 启动浏览�?        browser = await p.chromium.launch(headless=False, slow_mo=800)
        ...
        try:
            ...
            # 查找所有按�?            all_buttons = await page.locator('button').all()
            print(f"�?找到 {len(all_buttons)} 个按�?)
            
            # 3. 测试设置功能
            print("\n[3/12] 测试设置功能...")
            
            # 点击设置按钮 - 查找图标按钮或设置按�?            settings_button = None
            ...
            if settings_button:
                ...
                # 填写DeepSeek配置
                try:
                    # 尝试填写输入�?                    all_inputs = await page.locator('input').all()
                    ...
                    for i, inp in enumerate(all_inputs[:5]):
                        try:
                            ...
                            if 'password' in inp_type or 'key' in placeholder.lower() or 'key' in name.lower():
                                await inp.fill('sk-test-placeholder')
                    ...
                    # 保存设置 - 点击任意看起来像确认的按�?                    for selector in ['button:has-text("保存"), button:has-text("确认"), button:has-text("Save"), button.btn-primary']:
                        try:
            ...
            if create_button:
                ...
                # 填写项目信息
                try:
                    # 填写输入�?                    all_inputs = await page.locator('input, textarea').all()
            ...
            # 查找所有可点击的元�?            all_elements = await page.locator('[role="treeitem"], .file-tree *, .tree-node, .sidebar *').all()
            print(f"�?找到 {len(all_elements)} 个可能的文件�?)
            ...
            for selector in ['textarea, [contenteditable], .editor, .cm-content, .ProseMirror']:
                try:
                    ...
                    if await editor.count() > 0:
                        ...
                        # 输入一些内�?                        await page.keyboard.type('测试内容...', delay=100)
            ...
            for btn in all_buttons[-10:]:  # 只试后面的按�?                try:
                    await btn.click()
                    await asyncio.sleep(0.5)
                except:
                    continue
