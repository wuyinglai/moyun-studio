...
async def test_outline_fix():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # 写作风格: 快节奏
            print("  选择写作风格: 快节奏")
            style_btn = page.locator("button.btn-option", has_text="快节奏").first
            ...
            # 5. 点击"生成书名与创意"
            print("\n[5/11] 点击'生成书名与创意'...")
            generate_btn = page.locator("button.btn-primary", has_text="生成书名与创意").first
            ...
            while time.time() - start < 45:
                await asyncio.sleep(2)
                # 检查是否出现书名输入框或创意内容
                name_input = page.locator("input[value]").first
                ...
                if has_name:
                    ...
                    if name_val:
                        ...
                        break
                # 检查是否有创意描述文本框
                textarea = page.locator("textarea.form-textarea").first
            ...
            if book_name_found:
                ...
                report["errors"].append("书名生成可能未完成")
                print("⚠ 书名生成状态不确定")
            ...
            # 7. 点击"下一步：生成大纲"
            print("\n[7/11] 点击'下一步：生成大纲'...")
            next_btn = page.locator("button.btn-primary", has_text="下一步").first
            ...
            while time.time() - start < 120:
                ...
                if await title_el.count() > 0:
                    ...
                    if "确认" in title_text:
                        outline_done = True
                        print(f"  ✓ 大纲生成完成，进入确认步骤 ({time.time()-start:.1f}s)")
                        break
                
                # 检查是否显示成功
                if "创建成功" in text_content or "项目创建成功" in text_content:
                    outline_done = True
                    break
            ...
            if "待生成详细大纲" in text_content:
                print("✗ 大纲仍显示'待生成详细大纲'")
                report["errors"].append("大纲未实际生成，仍显示占位文本")
            else:
            ...
            if len(chapter_matches) > 0:
                print(f"✓ 检测到 {len(chapter_matches)} 个章节")
    ...
    print("测试报告")
    print("=" * 80)
    print(f"大纲生成成功: {'是' if report['outline_generated'] else '否'}")
    print(f"生成章节数: {report['chapter_count']}")
    print(f"包含详细内容: {'是' if report['has_detailed_content'] else '否'}")
    if report["errors"]:
        print(f"错误信息:")
